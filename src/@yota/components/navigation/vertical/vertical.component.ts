...
import { YotaNavigationItem, YotaVerticalNavigationAppearance, YotaVerticalNavigationMode, YotaVerticalNavigationPosition } from '@yota/components/navigation/navigation.types';
import { YotaVerticalNavigationAsideItemComponent } from '@yota/components/navigation/vertical/components/aside/aside.component';
...
import { YotaScrollbarDirective } from '@yota/directives/scrollbar/scrollbar.directive';
...
@Component({
    selector       : 'yota-vertical-navigation',
    templateUrl    : './vertical.component.html',
    styleUrls      : ['./vertical.component.scss'],
    animations     : yotaAnimations,
    encapsulation  : ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush,
    exportAs       : 'yotaVerticalNavigation',
    standalone     : true,
    imports        : [YotaScrollbarDirective, NgFor, NgIf, YotaVerticalNavigationAsideItemComponent, YotaVerticalNavigationBasicItemComponent, YotaVerticalNavigationCollapsableItemComponent, YotaVerticalNavigationDividerItemComponent, YotaVerticalNavigationGroupItemComponent, YotaVerticalNavigationSpacerItemComponent],
})
export class YotaVerticalNavigationComponent implements OnChanges, OnInit, AfterViewInit, OnDestroy
{
    ...
    @Input() appearance: YotaVerticalNavigationAppearance = 'default';
    @Input() autoCollapse: boolean = true;
    @Input() inner: boolean = false;
    @Input() mode: YotaVerticalNavigationMode = 'side';
    @Input() name: string = this._yotaUtilsService.randomId();
    @Input() navigation: YotaNavigationItem[];
    @Input() opened: boolean = true;
    @Input() position: YotaVerticalNavigationPosition = 'left';
    @Input() transparentOverlay: boolean = false;
    @Output() readonly appearanceChanged: EventEmitter<YotaVerticalNavigationAppearance> = new EventEmitter<YotaVerticalNavigationAppearance>();
    @Output() readonly modeChanged: EventEmitter<YotaVerticalNavigationMode> = new EventEmitter<YotaVerticalNavigationMode>();
    @Output() readonly openedChanged: EventEmitter<boolean> = new EventEmitter<boolean>();
    @Output() readonly positionChanged: EventEmitter<YotaVerticalNavigationPosition> = new EventEmitter<YotaVerticalNavigationPosition>();
    @ViewChild('navigationContent') private _navigationContentEl: ElementRef;

    activeAsideItemId: string | null = null;
    onCollapsableItemCollapsed: ReplaySubject<YotaNavigationItem> = new ReplaySubject<YotaNavigationItem>(1);
    onCollapsableItemExpanded: ReplaySubject<YotaNavigationItem> = new ReplaySubject<YotaNavigationItem>(1);
    ...
    private _yotaScrollbarDirectives!: QueryList<YotaScrollbarDirective>;
    private _yotaScrollbarDirectivesSubscription: Subscription;
    ...
    @HostBinding('class') get classList(): any
    {
        /* eslint-disable @typescript-eslint/naming-convention */
        return {
            'yota-vertical-navigation-animations-enabled'             : this._animationsEnabled,
            [`yota-vertical-navigation-appearance-${this.appearance}`]: true,
            'yota-vertical-navigation-hover'                          : this._hovered,
            'yota-vertical-navigation-inner'                          : this.inner,
            'yota-vertical-navigation-mode-over'                      : this.mode === 'over',
            'yota-vertical-navigation-mode-side'                      : this.mode === 'side',
            'yota-vertical-navigation-opened'                         : this.opened,
            'yota-vertical-navigation-position-left'                  : this.position === 'left',
            'yota-vertical-navigation-position-right'                 : this.position === 'right',
        };
        /* eslint-enable @typescript-eslint/naming-convention */
    }
    ...
    /**
     * Setter for yotaScrollbarDirectives
     */
    @ViewChildren(YotaScrollbarDirective)
    set yotaScrollbarDirectives(yotaScrollbarDirectives: QueryList<YotaScrollbarDirective>)
    {
        // Store the directives
        this._yotaScrollbarDirectives = yotaScrollbarDirectives;

        // Return if there are no directives
        if ( yotaScrollbarDirectives.length === 0 )
        {
            return;
        }

        // Unsubscribe the previous subscriptions
        if ( this._yotaScrollbarDirectivesSubscription )
        {
            this._yotaScrollbarDirectivesSubscription.unsubscribe();
        }

        // Update the scrollbars on collapsable items' collapse/expand
        this._yotaScrollbarDirectivesSubscription =
            merge(
                ...
                .subscribe(() =>
                {
                    ...
                    yotaScrollbarDirectives.forEach((yotaScrollbarDirective) =>
                    {
                        yotaScrollbarDirective.update();
                    });
                });
    }
    ...
    ngAfterViewInit(): void
    {
        ...
        setTimeout(() =>
        {
            ...
            if ( !this._navigationContentEl.nativeElement.classList.contains('ps') )
            {
                ...
                const activeItem = this._navigationContentEl.nativeElement.querySelector('.yota-vertical-navigation-item-active');
            ...
            else
            {
                ...
                this._yotaScrollbarDirectives.forEach((yotaScrollbarDirective) =>
                {
                    // Skip if not enabled
                    if ( !yotaScrollbarDirective.isEnabled() )
                    {
                        return;
                    }

                    // Scroll to the active element
                    yotaScrollbarDirective.scrollToElement('.yota-vertical-navigation-item-active', -120, true);
                });
            }
        });
    }
     ...
     * @param item
     */
    openAside(item: YotaNavigationItem): void
    {
     ...
     * @param item
     */
    toggleAside(item: YotaNavigationItem): void
    {
        // Toggle
    ...
    private _showOverlay(): void
    {
        ...
        this._overlay.classList.add('yota-vertical-navigation-overlay');

        // Add a class depending on the transparentOverlay option
        if ( this.transparentOverlay )
        {
            this._overlay.classList.add('yota-vertical-navigation-overlay-transparent');
        }
    ...
    private _showAsideOverlay(): void
    {
        ...
        this._asideOverlay.classList.add('yota-vertical-navigation-aside-overlay');
