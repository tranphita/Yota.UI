...
import { YOTA_MOCK_API_DEFAULT_DELAY } from '@yota/lib/mock-api/mock-api.constants';
import { YotaMockApiService } from '@yota/lib/mock-api/mock-api.service';
import { delay, Observable, of, switchMap, throwError } from 'rxjs';

export const mockApiInterceptor = (request: HttpRequest<unknown>, next: HttpHandlerFn): Observable<HttpEvent<unknown>> =>
{
    const defaultDelay = inject(YOTA_MOCK_API_DEFAULT_DELAY);
    const yotaMockApiService = inject(YotaMockApiService);

    // Try to get the request handler
    const {
        handler,
        urlParams,
    } = yotaMockApiService.findHandler(request.method.toUpperCase(), request.url);
